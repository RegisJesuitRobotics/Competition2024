...
public class AlternatePattern implements Pattern {
  ...
  public AlternatePattern(
      double pattern1Time, double pattern2Time, Pattern pattern1, Pattern pattern2) {
  ...
  public void applyTo(RaiderAddressableLEDBuffer buffer, double patternTime) {
    if (patternTime % (pattern1Time + pattern2Time) < pattern1Time) {
      pattern1.applyTo(
          buffer,
          patternTime - (pattern2Time * ((int) (patternTime / (pattern1Time + pattern2Time)))));
    } else {
      pattern2.applyTo(
          buffer,
          patternTime - (pattern1Time * ((int) (patternTime / (pattern1Time + pattern2Time)) + 1)));
    }
  }
}
