...
public class StructArrayTelemetryEntry<T> extends PrimitiveTelemetryEntry {
    ...
    public StructArrayTelemetryEntry(String path, Struct<T> struct, boolean shouldNT, boolean shouldLazyLog) {
        super(shouldLazyLog);

        logEntry = StructArrayLogEntry.create(DataLogManager.getLog(), path, struct);
        if (shouldNT) {
            networkPublisher = NetworkTableInstance.getDefault().getStructArrayTopic(path, struct).publish();
        } else {
    ...
    /**
     * Appends a value to the log. If lazy logging, only appends if the value is a different object from the last value, and they are not Arrays#equals.
