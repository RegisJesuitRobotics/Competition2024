...
public class RawTelemetryEntry extends PrimitiveTelemetryEntry {
  ...
  public RawTelemetryEntry(
      String typeString, String path, boolean shouldNT, boolean shouldLazyLog) {
    super(shouldLazyLog);

    logEntry = new RawLogEntry(DataLogManager.getLog(), path);
    if (shouldNT) {
      networkPublisher = NetworkTableInstance.getDefault().getRawTopic(path).publish(typeString);
